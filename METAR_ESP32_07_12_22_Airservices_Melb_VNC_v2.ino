...
Changes:
 - Change data source to NAIPS API (SOAP request) - NAIPS login details required
 - Change from ingesting 20 stations at once, to one at once (this would be a good target for further development as NAIPS can tolerate this - reduce API hits)
 - Remove most of the ParseMetar code and replace it with simpler code that decodes the METAR string and places it into the existing variables
 - Derive flight categories (VFR/MVFR/IFR/LIFR) from METAR data rather than provided information
 - Report temp in C instead of F
 - Change from 6 min to 15 min updates (reducing API hits)
 To do:
 - Save NAIPS username/password in EPROM and allow webpage to accept username/password change (rather than hardcoding here, which will need updating whenever the NAIPS password changes)
...
const char*    password = "WLAN AUTH";       // your network password

// Add NAIPS credentials
String authUsername = "NAIPS_USER_CASE_SENSITIVE";
String authPassword = "NAIPS_PASS_CASE_SENSITIVE";
  ...
  if ((wifiMulti.run() == WL_CONNECTED)) {
    ...
    if (httpCode > 0) {
      ...
      //  Serial.println(naips); //debugging - raw NAIPS location briefing
        naips.replace("SPECI","METAR"); // We don't really need to differentiate SPECI from METAR, so to simplify the code, replace instances of "SPECI" with "METAR"
...
// Set the stuff to 0 now
    ...
    // QNH section - the four characters after the first instance of "Q"
    String qnh;
    parsedmetar.remove(parsedmetar.length() - 1); // remove the last space at the end
    qnh = parsedmetar.substring(parsedmetar.indexOf(" Q") + 2,parsedmetar.indexOf(" Q") + 6);
   ...
   // If CAVOK is present in the METAR, set cloud type to "NCD" - near enough for our purposes and simplifies things
    ...
    // First, check the fifth character after "KT". If it is "V", it means variable wind direction is being reported, and we need to increase the substring offset by 8
    ...
    // Except, if the third character before "KT" is "G", that means we have wind gusts (first three chars are direction, next two chars are wind speed, then "G", then next two chars are gust spd, then "KT")
    String currentwinddir;
    String currentwindspd;
      
      if (parsedmetar.substring(parsedmetar.lastIndexOf("KT") - 3, parsedmetar.lastIndexOf("KT") - 2) != "G") {
        ...
        currentwindspd = parsedmetar.substring(parsedmetar.lastIndexOf("KT") - 5, parsedmetar.lastIndexOf("KT") - 3);
        currentwinddir = parsedmetar.substring(parsedmetar.lastIndexOf("KT") - 8, parsedmetar.lastIndexOf("KT") - 5);    
      }
      
      //Serial.println("QNH is: " + qnh);
          ...
          if (c == '\n') {                    // if the byte is a newline character
            ...
            if (currentLine.length() == 0) {
              ...
              if (summary_flag == 1)  {
                ...
                for (int i = 0; i < (No_Stations + 1); i++) {
                  ...
                  if (Stations[i].substring(0, 4) != "NULL")    {           // Display Station
                    ...
                    TempF = temp[i]; //* 1.8 + 32;  // deg F // Keep it in deg C please!
              ...
              if (station_flag == 1 || summary_flag == 1)  {
                ...
                TempF = temp[sta_n];// * //1.8 + 32;  // deg F // Keep it in C please!
                ...
                if (temp[sta_n] == 0 && dewpt[sta_n] == 0)  client.print(F("NA</FONT>")); else
                  client.print(String(temp[sta_n], 1) + " Deg C&nbsp&nbsp&nbsp:&nbsp&nbsp&nbsp" + String(TempF, 1) + " Deg C</FONT>");
